...
const BulkImportForm = ({ onSuccess }) => {
  ...
  const handleParse = (e) => {
    ...
    if (parsed.length === 0) {
      setError(
        "Could not parse any items. Please check the format and try again."
      );
      return;
    }
    ...
    try {
      const purchasePromises = itemsToReview.map((item) => {
        const purchaseData = {
          ...
          isStockItem: true, // fix: ensure bulk-imported items are marked as stock items by default
        };
      ...
      {view === "review" && (
        <div>
          <p className="mb-4 text-sm text-gray-600">
            Review the parsed items below. If everything looks correct, confirm
            the import.
          </p>
          ...
          <div className="pt-6 flex justify-between items-center">
            ...
            <Button onClick={handleConfirmAndSave} disabled={isSubmitting}>
              {isSubmitting
                ? "Importing..."
                : `Confirm & Save ${itemsToReview.length} Items`}
            </Button>
          </div>
        </div>
      )}
