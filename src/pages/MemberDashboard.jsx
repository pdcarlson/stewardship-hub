...
export const MemberDashboardUI = ({
  ...
  const activeItems = useMemo(() => {
    // fix: show items unless they are explicitly marked as not a stock item.
    // this ensures that older items without this flag are still visible.
    const visibleItems = purchases.filter((p) => p.isStockItem !== false);
  ...
  const shoppingListNames = useMemo(
    () => new Set(shoppingList.map((item) => item.itemName)),
    [shoppingList]
  );
  const suggestionModalTitle = editingSuggestion
    ? "Edit Suggestion"
    : "Suggest a New Item";
            ...
            <div className="max-w-3xl mx-auto space-y-8">
              <Card>
                ...
                <p className="text-gray-600 mb-6">
                  Search for an item to see if we have it. If something is out
                  of stock, let the steward know.
                </p>
                <div>
                  <input
                    type="text"
                ...
                <div className="flex justify-between items-center mb-6">
                  <div>
                    ...
                    <p className="text-gray-600">
                      Suggest new items for the house or manage your existing
                      suggestions.
                    </p>
                  </div>
                  <Button onClick={() => setIsSuggestionModalOpen(true)}>
                    Suggest an Item
                  </Button>
                </div>
                <div className="space-y-3">
                            ...
                            <div className="flex-1">
                              ...
                              {suggestion.adminResponse && (
                                <p className="text-sm italic text-indigo-600 mt-2">
                                  Steward's Response: {suggestion.adminResponse}
                                </p>
                              )}
                            </div>
                            <div className="flex items-center space-x-3 ml-4">
                              <span
                                className={`px-2 py-1 text-xs font-semibold rounded-full ${statusColors[suggestion.status]}`}
                              >
                                {suggestion.status}
                              </span>
